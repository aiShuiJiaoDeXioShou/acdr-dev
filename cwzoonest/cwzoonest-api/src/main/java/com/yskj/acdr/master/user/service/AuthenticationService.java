...
import com.yskj.acdr.master.user.entity.UserIdentityVerification;
import com.yskj.acdr.master.user.entity.Users;
import com.yskj.acdr.master.user.mapper.UserIdentityVerificationMapper;
...
public class AuthenticationService {
    ...
    public AuthenticationErrorCode AuthId(MultipartFile file) {
        ...
        String flag = File.separator + petId;
        if (file != null) {
            // 上传图片
            String fileName = FileUtils.uploadSingleFile(file, profile, flag);
            // 图片上传之后，对图片进行识别, 识别正面
            Map<String, Object> faceFes = recognizeIDCard(profile + fileName, "face");
            // 对图片中的身份信息进行识别
            AuthenticationResponse authRes = authenticate((String) faceFes.get("name"), (String) faceFes.get("num"));
            AuthenticationErrorCode code = AuthenticationErrorCode.fromCode(authRes.getRespCode());
            // 判断是否验证成功
            if (code.getCode().equals("0000")) {
                ...
                var data = new UserIdentityVerification()
                        ...
                        .setIsFake((String) faceFes.get("is_fake"));
                uidmapper.insert(data);
            }
            return code;
        }
