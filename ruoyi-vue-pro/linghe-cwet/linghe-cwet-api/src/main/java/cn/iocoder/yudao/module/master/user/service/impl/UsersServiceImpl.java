...
import cn.iocoder.yudao.module.common.cache.GlobalRedisCache;
...
import cn.iocoder.yudao.module.common.verify.PhoneValidator;
...
import cn.iocoder.yudao.module.utils.ProfileUtil;
...
public class UsersServiceImpl extends ServiceImpl<UsersMapper, Users> implements UsersService {
    ...
    public GlobalResponse<String> getCode(String phone) {
        ...
        if (object.getInt("error_code") == 0 || ProfileUtil.isDebug()) {
            ...
            return GlobalResponse.success("验证码获取" + GlobalConstant.SUCCESS, code.substring(7));
        } else {
            log.debug("验证码获取失败-" + object.get("error_code") + ":" + object.get("reason"));
            return GlobalResponse.failure("验证码获取" + GlobalConstant.FAILURE);
        }
    }
    ...
    public GlobalResponse<String> login(String phone, String code) {
        ...
        // 使用 SaToken 进行登录
        LoginUtil.login(user.getId());

        // 返回成功响应
        return GlobalResponse.success("登录成功", LoginUtil.getTokenValue());
    }
    ...
    public Users getSafeUsers() {
        Users users = baseMapper.selectById(LoginUtil.getLoginIdAsLong());
        return users.setOpenid("")
                .setCreateTime(null)
                .setUpdateTime(null)
                .setEmail("")
